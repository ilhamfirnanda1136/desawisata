...
class apiController extends Controller
{
    ...
    public function apiDesa($pusatid)
    {
        $wisata = wisata::with(['kecamatan'=>function($query){
            $query->with('kota');
        },'pusat'])->where('pusat_id',$pusatid)->orderBy('nama_desa','asc')->get();
        if($wisata->count() > 0) return response()->json(['status'=>200,'wisata'=>$wisata]);
        return response()->json(['status' =>404]);
    }

    public function pendampingAll()
    {
        $pendamping = pendamping::orderBy('nama_pendamping','asc')->paginate(3);
        if($pendamping->count() > 0) return response()->json(['status'=>200,'pendamping'=>$pendamping]);
        return response()->json(['status' =>404]);
    }
}
