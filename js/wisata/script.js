class wisata {
  ...
  static simpan = async (edit = false) => {
    ...
    // const form = document.querySelector(this.DOMStrings.formWisata)
    console.log(form)
      ...
      .then((datas) => {
        ...
        kecamatan.forEach((data) => {
          $(this.DOMStrings.editKecamatan).append(
            $('<option>').text(data.nama_kecamatan).attr('value', data.id),
          )
        })
        kota.forEach((data) => {
          $(this.DOMStrings.editKota).append(
            $('<option>').text(data.nama_kota).attr('value', data.id),
          )
        })
        ...
        for (const key in wisatas) {
          if (!notAllowed.includes(key)) {
            document.getElementById(`edit_${key}`).value = wisatas[key]
          }
        }
      })
